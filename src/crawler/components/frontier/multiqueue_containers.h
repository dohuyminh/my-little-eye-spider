...
concept QueueType = requires(T t) {
  ...
  // single-element enqueue accepts universal references (both lvalue and
  // rvalue)
  {
    t.enqueue(std::declval<const typename T::value_type&>())
  } -> std::same_as<void>;
  ...
  {
    t.enqueue_bulk(std::declval<const std::vector<typename T::value_type>&>())
  } -> std::same_as<void>;
  {
    t.enqueue_bulk(std::declval<std::vector<typename T::value_type>&&>())
  } -> std::same_as<void>;

  // bulk dequeue returns a vector of value_type
  {
    t.dequeue_bulk(std::declval<std::size_t>())
...
class MultiQueueContainers {
 public:
  ...
  std::size_t numQueues() const noexcept { return queues_.size(); }
  ...
  void enqueue_bulk(std::size_t queueIndex,
  ...
  void enqueue_bulk(std::size_t queueIndex, std::vector<ValueType>&& values) {
    ...
    queues_[queueIndex].enqueue_bulk(
        std::forward<std::vector<ValueType>>(values));
  }

  std::vector<ValueType> dequeue_bulk(std::size_t queueIndex,
                                      std::size_t count) {
    if (queueIndex >= queues_.size()) {
      throw std::invalid_argument("Invalid queue index");
    }
...
class StdQueueWrapper {
 public:
  ...
  void enqueue(const T& value) { queue_.push(value); }

  void enqueue(T&& value) { queue_.push(std::move(value)); }
...
class MoodyCamelConcurrentQueueWrapper {
 public:
  ...
  void enqueue(const T& value) { queue_.enqueue(value); }
